...
class UNA_score(DPOTrainer):
    ...
    def dpo_loss(
        ...
        if self.loss_type == "sigmoid":
            ###################################################### Deal with Dummy Answer
            if chosen_rating >= 0 and rejected_rating >= 0:
                print(1)
                losses = torch.sum(torch.square(chosen_logps - chosen_rating) + torch.square(rejected_logps - rejected_rating))
            elif chosen_rating >= 0:
                print(2)
                losses = torch.sum(torch.square(chosen_logps - chosen_rating))
            elif rejected_rating >= 0:
                print(3)
                losses = torch.sum(torch.square(rejected_logps - rejected_rating))
            ###################################################### Deal with Dummy Answer
        else:
            raise ValueError(
...
output_dir = "./una_score_MSE_3e-5_0.03"
max_seq_length = 2048
...
# Base template
BASE_TEMPLATE = '''<s>[INST] {prompt} [/INST]'''
...
def apply_formatting(example, tokenizer):
    # rating is from 1 to 5: normalize them
    prompt = BASE_TEMPLATE.format(prompt=example["prompt"])
    chosen_completion = RESPONSE_TEMPLATE.format(response=example["chosen"][-1]["content"])
    chosen_rating = (example["score_chosen"] - 1) / 9
    rejected_completion = RESPONSE_TEMPLATE.format(response=example["rejected"][-1]["content"])
...
def load_and_format_datasets(input_data_dir, tokenizer):
    ...
    '''
    train_dataset = dataset_dict_clean['train']
    chosen = [example['chosen-rating'] for example in train_dataset]
    # Get the maximum value
    max_value = max(chosen)
    print(f'maximum value: {max_value}')
    
    rejected = [example['rejected-rating'] for example in train_dataset]
    # Get the maximum value
    min_value = min(rejected)
