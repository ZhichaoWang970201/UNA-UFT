    ...
    def dpo_loss(
        ...
        if self.loss_type == "sigmoid":
            ...
            if chosen_rating >= 0 and rejected_rating >= 0:
                print(1)
                losses = torch.sum(torch.square(chosen_logps - 1) + torch.square(rejected_logps - 0))
            elif chosen_rating >= 0:
                print(2)
                losses = torch.sum(torch.square(chosen_logps - 1))
            elif rejected_rating >= 0:
                print(3)
                losses = torch.sum(torch.square(rejected_logps - 0))
            ############################################################ 
        else:
            raise ValueError(
...
output_dir = "./una_binary_MSE_0.01"
max_seq_length = 2048
